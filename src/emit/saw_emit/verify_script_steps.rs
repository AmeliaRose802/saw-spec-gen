...
use super::verify_script_preconds::{emit_param_preconditions, emit_param_preconditions_filtered};
...
pub(super) fn emit_equiv_spec_body(
    ...
    // Per-parameter `llvm_precond` clauses are collected here and flushed
    // *after* the loop: a buffer sized by a sibling length param (`m`
    // sized by `_In_reads_(nm)`) references a fresh var declared later,
    // so emitting inline would place the precond before `nm <-
    ...
    for (i, param) in target_spec.params.iter().enumerate() {
        ...
        if is_in_buf || is_out_buf {
            ...
            emit_param_preconditions_filtered(
                &mut deferred_preconds,
                &param.preconditions,
                &param.name,
            );
        ...
        match param.alloc_type {
            AllocType::FreshVar => {
                ...
                emit_param_preconditions(&mut deferred_preconds, &param.preconditions);
    ...
    // Flush the deferred per-parameter preconditions now that every
    // param's fresh var is in scope (see `deferred_preconds`).
    out.push_str(&deferred_preconds);
