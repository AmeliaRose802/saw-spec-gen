...
/// Bit width of an `llvm_int N` SAW type string, or `None` for any
/// other shape (arrays, aliases, pointers). Used to size the Cryptol
/// literal in a status-primitive success-sentinel return.
fn int_bits(saw_type: &str) -> Option<u32> {
    saw_type.strip_prefix("llvm_int ")?.trim().parse().ok()
}
...
pub fn generate_unspecified_spec(spec: &SpecConstraint, all_globals: &[GlobalVarInfo]) -> String {
    ...
    if !crate::emit::saw_emit::writer::is_void_saw_type(&spec.return_constraint.saw_type) {
        if spec.return_constraint.returns_pointer {
            ...
            // and friends) report `_Thrd_result`, whose success value
            // `_Thrd_success` is `0`. A fresh-symbolic return lets the
            // solver pick a failure code, sending a lock-guarded body
            // down its `_Throw_Cpp_error` → `unreachable` path so the
            // subgoal fails. Pin the success sentinel instead — sound
