<#
.SYNOPSIS
    Run the SAW demo test suite declared in tests/saw_demos/cases.psd1.
...
.EXAMPLE
    pwsh tests/saw_demos/Run-SawDemos.ps1 -Tag cpp_havoc,rust_havoc

.EXAMPLE
    $env:SKIP_SAW_TESTS = '1'; pwsh tests/saw_demos/Run-SawDemos.ps1   # no-op
#>
[CmdletBinding()]
param(
    [string[]]$Tag,
...
if ($env:SKIP_SAW_TESTS -eq '1') {
    Write-Host "SKIP_SAW_TESTS=1 set; skipping SAW demo suite." -ForegroundColor Yellow
    exit 0
}

# ── Locate repo root (this script lives at tests/saw_demos/). ──────────────
...
Write-Host ""
Write-Host "SAW demo suite: $total case(s)" -ForegroundColor Cyan
Write-Host ('-' * 60)
