// DEMO: Problem 2 (safe_mul) — DISPROVED reference (Rust).
//
// We deliberately use a *different* (simpler) bug than the literal
// Python-style C++ version in safe_mul_disproved.cpp. The Python check
...
// verdict. To keep the demo deterministic the Rust disproved version
...
// sibling file safe_mul_disproved.cpp for the parity demo.)
//
