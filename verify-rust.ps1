...
# its combined verdict. Shape kept in sync with verify.ps1.
function Write-ResultJson($verdict, $cex, $expected, $actual) {
    $payload = [PSCustomObject]@{
        ...
        verdict        = $verdict
        counterexample = $cex      # ordered list of @{ name=...; value=... }
        expected       = $expected # Cryptol value at cex inputs (as string)
        actual         = $actual   # Rust value at cex inputs   (as string)
    }
...
"@ | Set-Content $evalScript -Encoding utf8
    ...
    # We `include!()` the user's source verbatim into a throwaway crate so the
    # function (even if private) is visible. Each cex value is cast through
    # `as <RustType>` so we don't need to match Rust's exact param type.
    $actualVal = $null
    if ($cexPairs.Count -gt 0 -and $cexPairs.Count -eq $rustParamTypes.Count) {
        ...
        $rustFileEsc = $RustFile.ToString().Replace('\', '\\')
        @"
// Auto-generated by verify-rust.ps1: calls ${Function} at the SAW
// counterexample inputs so we can show the actual Rust return value.
include!(r"$RustFile");

#[allow(dead_code)]
fn main() {
    let r = ${Function}($($callArgs -join ', '));
    println!("RUST_RESULT={}", r);
}
