...
On x86_64 MSVC, aggregates > 8 bytes are returned via a hidden
`sret` output pointer.  The Cryptol model takes a trailing [12][8]
pre-state parameter representing the `body` field (at offset 4),
while the full struct is 16 bytes.  saw-spec-gen must:

  1. Allocate preBytes at the FULL buffer size (16 bytes).
  2. Pass `(take`{12} (drop`{4} preBytes))` — not the raw preBytes —
